...
static bool validateScheme(boost::string_view url) {
...
void ParseURL::parse(bool strict) noexcept {
  ...
  if (validateScheme(url_)) {
    ...
    if (valid_) {
      ...
      scheme_ = url_.substr(u.field_data[UF_SCHEMA].off,
      ...
      path_ =
          url_.substr(u.field_data[UF_PATH].off, u.field_data[UF_PATH].len);
      query_ =
...
void ParseURL::parseNonFully(bool strict) noexcept {
  ...
  auto temp_str = url_.substr(0, authorityEnd);
  authority_ = std::string(temp_str.begin(), temp_str.end());
  ...
  if (hashStart != std::string::npos) {
    fragment_ = url_.substr(hashStart + 1, std::string::npos);
  }
...
bool ParseURL::parseAuthority() noexcept {
  ...
  if (pos != std::string::npos) {
    try {
        auto temp_value = std::string(authority_, pos + 1, std::string::npos);
        port_ = std::stoi(temp_value);
/*      port_ = folly::to<uint16_t>(
          std::string(authority_, pos + 1, std::string::npos));*/
    } catch (...) {
      return false;
    }
  }
  ...
  if (left == std::string::npos && right == std::string::npos) {
    // not a ipv6 literal
    host_ = std::string(authority_, 0, pos);
    return true;
  } else if (left < right && right != std::string::npos) {
    // a ipv6 literal
    host_ = std::string(authority_, left, right - left + 1);
    return true;
  } else {
    return false;
  }
...
folly::Optional<std::string> ParseURL::getQueryParam(
